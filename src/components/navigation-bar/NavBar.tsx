...
const NavBar: React.FC<TypeProps> = (props): ReactElement => {
  ...
  function updateSlide(selectedSlideIndex: number): void {
    ...
    if (selectedSlideIndex === 0) {
      ...
      updateDocTitle(
        'WebFace • ' + pageName.charAt(0).toUpperCase() + pageName.slice(1)
      );
    }
  }
  ...
  for (let i = 0; i < pageList.length; i++) {
    if (i === 0) {
      ...
      buttons[i] = (
        <button
          key={i}
          id="logo-color-change"
          onClick={updateSlide.bind(null, i)}
        >
          <img
            className={
      ...
      buttons[i] = (
        <button key={i} onClick={updateSlide.bind(null, i)}>
          <span
            className={
              i === slideIndex ? 'nav-item active' : 'nav-item inactive'
            }
  ...
  return (
    <header className="nav">
      <div className="nav-container">
        <div className="nav-item logo">{buttons[0]}</div>
        <div className="nav-item links">{buttons.slice(1) /* 1 ~ end */}</div>
      </div>
    </header>
  );
};
...
function updateHeight(indexToGetHeight: number): void {
  ...
  const selectedSlide: Element =
    document.querySelectorAll('.slide-content')[indexToGetHeight];
...
function updateDocTitle(text: string) {
  document.title = text;
}
